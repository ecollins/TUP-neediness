...
% For 'std/echo', 'error-based refine' or 'error-based expand'
   ...
   while i<=n,
      ...
      if i==1 && k==0,
         ...
         %%% EAL December 2015: Add code to deal with possible
         %%% missing values in A.  Approach only works if we update
         %%% one column at a time.

         if kstart==1; % We're presumed to be in missing-data
                       % correction mode
           SEQKL_U(1:m,k+1:k+lup) = fixmiss(A(:,i:ip),S);
         else;
